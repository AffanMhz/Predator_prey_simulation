...
function drawGrid(){ gctx.clearRect(0,0,gC.width,gC.height); const cell = Math.min(gC.width,gC.height)/GRID_SIZE; for(let i=0;i<GRID_SIZE;i++)for(let j=0;j<GRID_SIZE;j++){ switch(grid[i][j]){case GRASS: gctx.fillStyle='#8BC34A'; break; case RABBIT: gctx.fillStyle='#FFB74D'; break; case FOX: gctx.fillStyle='#E57373'; break; default: gctx.fillStyle='#041427'} gctx.fillRect(i*cell,j*cell,Math.ceil(cell),Math.ceil(cell)); }}

function drawTime(){ tctx.clearRect(0,0,tC.width,tC.height); tctx.fillStyle='rgba(255,255,255,0.08)'; tctx.fillRect(0,0,tC.width,tC.height); tctx.fillStyle='white'; tctx.fillText('Rabbits',8,14); if(populationHistory.rabbits.length>1){ let max=Math.max(...populationHistory.rabbits,1); let stepX=tC.width/populationHistory.rabbits.length; tctx.strokeStyle='#FFB74D'; tctx.beginPath(); for(let i=0;i<populationHistory.rabbits.length;i++){ let x=i*stepX, y=tC.height- (populationHistory.rabbits[i]/max)*tC.height; if(i===0) tctx.moveTo(x,y); else tctx.lineTo(x,y);} tctx.stroke(); } }

function drawPhase(){ pctx.clearRect(0,0,pC.width,pC.height); if(populationHistory.rabbits.length<10) return; let maxR = Math.max(...populationHistory.rabbits), maxF=Math.max(...populationHistory.foxes); pctx.strokeStyle='#8fb4ff'; pctx.beginPath(); for(let i=0;i<populationHistory.rabbits.length;i++){ let x=(populationHistory.rabbits[i]/maxR)*pC.width, y=pC.height - (populationHistory.foxes[i]/maxF)*pC.height; if(i===0) pctx.moveTo(x,y); else pctx.lineTo(x,y);} pctx.stroke(); }
